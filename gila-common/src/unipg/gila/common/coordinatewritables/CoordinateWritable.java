...
public class CoordinateWritable extends MiniCoordinateWritable {
  ...
  /**
   * A set which contains all the ids of the vertices already taken into
   * account.
   */
   ...
   * 
   * @param neigh
   *          The vertex id to be set as analyzed.
   */
  ...
  /**
   * At the end of a drawing cycle (seeding + propagation) this method resets
   * the Analyzed set.
   */
  ...
  /**
   * When called, it means that for that vertex attractive forces have been
   * already computed.
   */
  ...
  /**
   * Adds to the internal force vector the values in the given 2-dimensional
   * array.
   * 
   * @param force
  ...
  // int capacity = messageStack.isEmpty() ? 0 : Math.min(messages,
  // messageStack.size());
   ...
   * 
   * @see
   * unipg.gila.common.coordinatewritables.MiniCoordinateWritable#newInstance()
   */
  @Override
