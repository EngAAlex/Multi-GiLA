...
public class SolarMessage
        extends
        MessageWritable<LayeredPartitionedLongWritable, LayeredPartitionedLongWritable> {
  ...
  public SolarMessage(LayeredPartitionedLongWritable payloadVertex,
          LayeredPartitionedLongWritable value, CODE code) {
  ...
  public SolarMessage(LayeredPartitionedLongWritable payloadVertex, int ttl,
          LayeredPartitionedLongWritable value, CODE code) {
  ...
  public SolarMessage(LayeredPartitionedLongWritable payload, int ttl,
          LayeredPartitionedLongWritable valueSun, ReferrersList extraPayload) {
  ...
  public SolarMessage copy() {
    SolarMessage tmp = new SolarMessage(this.payloadVertex.copy(), ttl, this
            .getValue().copy(), this.code);
  ...
  public MessageWritable<LayeredPartitionedLongWritable, LayeredPartitionedLongWritable> propagate() {
    SolarMessage toReturn = new SolarMessage(getPayloadVertex().copy(),
            getTTL() - 1, getValue().copy(), getCode());
  ...
  public String toString() {
    return "code " + code.toString() + " payload " + getPayloadVertex()
            + " value " + getValue() + " weight " + weight;
  }
