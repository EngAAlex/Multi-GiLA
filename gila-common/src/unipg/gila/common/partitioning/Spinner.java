...
/**
 * This class contains the implementation of the Spinner partitioner. It has
 * been left largely untouched, but the "master" computation and other methods
 * have been modified to adapt to Dafne's algorithm.
 * 
 * Modifications include the code for pruning the graph and the correct order of
 * computations in order to complete the the pre-process stage. Data structures
 * have been updated to match the ones used in Dafne.
 * 
...
public class Spinner {
  ...
  public static class ComputeNewPartition
    ...
    private double computeW(int newPartition) {
      return new BigDecimal(((double) loads[newPartition]) / totalCapacity)
    ...
    private void updateNeighborsPartitions(
      ...
      for (PartitionMessage message : messages) {
        ...
        vertex.setEdgeValue(otherId, new EdgeValue(message.getValue(),
                oldValue.getWeight()));
      }
    }
    ...
    public void preSuperstep() {
      ...
      numberOfPartitions = (short) getContext().getConfiguration().getInt(
              NUM_PARTITIONS, DEFAULT_NUM_PARTITIONS);
      repartition = (short) getContext().getConfiguration().getInt(
              REPARTITION, DEFAULT_REPARTITION);
      lambda = getContext().getConfiguration()
              .getFloat(LAMBDA, DEFAULT_LAMBDA);
      ...
      totalCapacity = (long) Math
              .round(((double) getTotalNumEdges() * (1 + additionalCapacity) / (numberOfPartitions + repartition)));
  ...
  public static class ConverterPropagate
    ...
    public void compute(
      ...
      sendMessageToAllEdges(vertex, new DegMessage(vertex.getId().get(),
              vertex.getNumEdges()));
    }
  }
  ...
  public static class Repartitioner
    ...
    public void compute(
      ...
      if (repartition < 0) {
        ...
        throw new RuntimeException("Repartitioner called with " + REPARTITION
                + " set to 0");
      }
      aggregate(loadAggregatorNames[partition],
              new LongWritable(vertex.getNumEdges()));
  ...
  public static class ConverterUpdateEdges
    ...
    public void compute(
      ...
      if (vertex.getNumEdges() == 1 || vertex.getNumEdges() == 0)
        return;
      for (DegMessage other : messages) { // Here one degree vertices are
                                          // pruned.
        if (pruneOneDegrees && other.getValue() == 1) {
          vertex.getValue().addOneEdge(other.getPayloadVertex());
          removeEdgesRequest(vertex.getId(),
                  new LongWritable(other.getPayloadVertex()));
          removeEdgesRequest(new LongWritable(other.getPayloadVertex()),
                  vertex.getId());
          removeVertexRequest(new LongWritable(other.getPayloadVertex()));
          continue;
        }
        EdgeValue edgeValue = vertex.getEdgeValue(new LongWritable(other
                .getPayloadVertex()));
        if (edgeValue == null) {
          ...
          vertex.setEdgeValue(new LongWritable(other.getPayloadVertex()),
                  edgeValue);
        }
      }
    }
    ...
    @Override
    public void preSuperstep() {
      edgeWeight = (byte) getContext().getConfiguration().getInt(EDGE_WEIGHT,
              DEFAULT_EDGE_WEIGHT);
  ...
  public static class PartitionerMasterCompute extends DefaultMasterCompute {
    ...
    public void initialize() throws InstantiationException,
      ...
      repartition = (short) getContext().getConfiguration().getInt(
              REPARTITION, DEFAULT_REPARTITION);
      windowSize = (int) getContext().getConfiguration().getInt(WINDOW_SIZE,
              DEFAULT_WINDOW_SIZE);
      ...
      registerAggregator(ConnectedComponentsComputation.activityAggr,
              BooleanAndAggregator.class);

      findComponents = getConf()
              .getBoolean(Spinner.findComponentsString, true);
    ...
    private void printStats(int superstep) {
      ...
      if (superstep > 2) {
        switch (superstep % 2) {
        case 0:
          System.out.println(((double) localEdges) / getTotalNumEdges()
                  + " local edges");
    ...
    private boolean algorithmConverged(int superstep) {
      ...
      if (superstep > 3 + windowSize) {
        ...
        System.out.println("BestState=" + best + " NewState=" + newState + " "
                + step);
        states.removeFirst();
      } else {
        System.out.println("BestState=" + newState + " NewState=" + newState
                + " " + 1.0);
      }
    ...
    private void setCounters() {
      ...
      getContext().getCounter(COUNTER_GROUP, MIGRATIONS_COUNTER).increment(
              totalMigrations);
      getContext().getCounter(COUNTER_GROUP, ITERATIONS_COUNTER).increment(
              getSuperstep());
    ...
    private void findComponentsMasterCompute(int superstep) {
      ...
      boolean lastActivity = ((BooleanWritable) getAggregatedValue(ConnectedComponentsComputation.activityAggr))
              .get();
